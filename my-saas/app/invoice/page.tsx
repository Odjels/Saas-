...
export default function InvoiceGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invoice = { clientName, clientEmail, date, dueDate, items, notes };
  ...
  const downloadPDF = async (invoiceId: string, template: Template) => {
    try {
      const res = await fetch(
        `/api/invoice/download/${invoiceId}?template=${template}`
      );
  ...
  const getStatusBadge = (status: InvoiceStatus) => {
    ...
    > = {
      PAID: { icon: <CheckCircle className="w-4 h-4" />, color: "bg-green-100 text-green-700 border-green-200", text: "Paid" },
      PENDING: { icon: <Clock className="w-4 h-4" />, color: "bg-yellow-100 text-yellow-700 border-yellow-200", text: "Pending" },
      OVERDUE: { icon: <AlertCircle className="w-4 h-4" />, color: "bg-red-100 text-red-700 border-red-200", text: "Overdue" },
      CANCELLED: { icon: <XCircle className="w-4 h-4" />, color: "bg-gray-100 text-gray-700 border-gray-200", text: "Cancelled" },
    };
